...
@njit(fastmath=True)
def fast_peak_fit(img, targ_par, xmin, xmax, ymin, ymax, cpar, num_cam, pix):
    """Fit the peaks in the image to a Gaussian."""
    imx, imy = cpar.imx, cpar.imy
    n_peaks = 0
    n_wait = 0
    x8, y8 = [0, 0, 0, 0], [0, 0, 0, 0]
    p2 = 0
    thres, disco = targ_par.gvthres[num_cam], targ_par.discont
                ...
                while n_wait > 0:
                    ...
                    for k in range(4):
                        ...
                        if gv > thres and \
                            ...
                            yn < ymax - 1 and \
                            gv <= gvref + disco and \
                            gvref + disco >= img[imx * (yn - 1) + xn] and \
                            gvref + disco >= img[imx * (yn + 1) + xn] and \
                            gvref + disco >= img[imx * yn + (xn - 1)] and \
    ...
    for i in range(n_peaks):
        ...
        if peaks[i].ymax == ymax - 1 and (xmax - xmin) > 32:
            continue
        if peaks[i].unr == 0 and \
            peaks[i].sumg > targ_par.sumg_min and \
            (peaks[i].xmax - peaks[i].xmin + 1) >= targ_par.nxmin and \
            (peaks[i].ymax - peaks[i].ymin + 1) >= targ_par.nymin and \
            (peaks[i].xmax - peaks[i].xmin) < targ_par.nxmax and \
            (peaks[i].ymax - peaks[i].ymin) < targ_par.nymax and \
            peaks[i].n >= targ_par.nnmin and \
...
def check_touch(tpeak, p1, p2):
    """Check whether two peaks are touching."""
    done = False
    ...
    if not done:
        ...
        tpeak.n_touch = min(tpeak.n_touch, 3)
...
def peak_fit_new(
    ...
    for i in range(num_objects):
        ...
        peak = Peak()
        peak.x, peak.y = int(round(x)), int(round(y))
        peak.sumg = intensity
        peaks.append(peak)
    return peaks
